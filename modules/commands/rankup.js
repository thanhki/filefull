...
module.exports.handleEvent = async function({ api, event, Currencies, Users }) {
	...
	if (level > curLevel && level != 1) {
		...
		var messsage = (typeof thread.customRankup == "undefined") ? msg = "Trình độ chém gió của {name} đã đạt tới level {level}, hãy tương tác nhiều để lên level nhé UwU" : msg = thread.customRankup,
			arrayContent;
		...
		if (existsSync(__dirname + "/rankup/mp4")) mkdirSync(__dirname + "/rankup/mp4", { recursive: true });
		if (existsSync(__dirname + `/rankup/${event.threadID}.mp4`)) arrayContent = { body: messsage, attachment: createReadStream(__dirname + `/cache/rankup/${event.threadID}.mp4`), mentions: [{ tag: name, id: senderID }] };
